...
if [[ "$config_path" == "" ]]; then 
	config_path="${HOME}/.rofi-neovim-launcher.config.json"
...
function get_projects {
	echo $(echo $config | jq -r -c '.[]' | wc -l) 
	projects_num=$(echo $config | jq -r -c '.[]' | wc -l )	
	
	if [[ $projects_num -gt 10 ]]; then projects_num=10; fi

	program=$({ echo $config | jq -r -c '.[]' | sort; echo $ADD_OPTION; echo $REMOVE_OPTION; } | rofi -dmenu -i -theme $theme -l $projects_num -p " NeoVim:")
	
	echo $(basename $program)
	echo $(realpath $program)
	command="alacritty -e tmuxinator start NeoVim $(realpath $program) $(basename $program) & disown"
